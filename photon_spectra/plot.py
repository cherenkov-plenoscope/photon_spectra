...
for name in equal["transmissivity"]:
    (leg,) = plt.plot(wavelengths, equal["transmissivity"][name], label=name)
...
for mirror_name in equal["reflectivity"]:
    (leg,) = plt.plot(
        wavelengths, equal["reflectivity"][mirror_name], label=mirror_name
    )
    legend_handles.append(leg)
ax.set_xlim([200e-9, 700e-9])
ax.set_xlabel("Wavelength / m")
...
for name in equal["photon_detection_efficieny"]:
    if "ccd" in name:
        (leg,) = plt.plot(
            wavelengths, equal["photon_detection_efficieny"][name], label=name
        )
        legend_handles.append(leg)
ax.set_xlim([200e-9, 700e-9])
ax.set_xlabel("Wavelength / m")
